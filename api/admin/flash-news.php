...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $result = $conn->query("SELECT id, text, link, active, createdAt FROM flash_news ORDER BY createdAt DESC");
...
// Handle POST request - create new flash news
    ...
    $stmt->bind_param("ssi", $data['text'], $link, $active);
    
    if ($stmt->execute()) {
        $id = $stmt->insert_id;
        
        // Get the created flash news
        $result = $conn->query("SELECT id, text, link, active, createdAt FROM flash_news WHERE id = $id");
        $flashNews = $result->fetch_assoc();
        
        // Return created flash news
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            ...
            if ($result && $result->num_rows > 0) {
                ...
                // Return updated flash news
                http_response_code(200);
                echo json_encode($flashNews);
            } else {
                // Return not found
                http_response_code(404); // Not Found
                echo json_encode(['error' => 'Flash news not found after update']);
            }
        } else {
            ...
            if ($result && $result->num_rows > 0) {
                ...
                // Return flash news with no changes
                http_response_code(200);
                echo json_encode($flashNews);
            } else {
                // Return not found
